...
def getDefinedStrings(filepath):
    ...
    with open(filepath, 'r', encoding="latin-1") as file:
        content = file.read()
        srch = re.compile('Key ID\=\"(STR_Bangalore_[_a-zA-Z0-9]*)"', re.IGNORECASE)
...
def main(argv):
    ...
    if ("str_KAT_tagging_name" in allUsedStrings): allUsedStrings.remove("str_KAT_tagging_name") # Handle tagging macro
